...
public abstract class choiceEditor extends FieldEditor {
	...
	static String[] _params = { "default", "empty", "type", "size",
			"labelSeparator", "elementSeparator" };

	static String[][] _paramValues = { null, null,
			{ "hidden", "radio", "checkbox", "tickbox" }, null, null, null };
	...
	public abstract Object getOptions(RecordFormatter rf, int fieldIndex,
			Dictionary formatParams);

	/** Gets the number of available options. */
	public abstract int getOptionsLength(RecordFormatter rf, int fieldIndex,
			Object opts);

	/** Gets the value of option 'i'. */
	public abstract Object getOptionValue(RecordFormatter rf, int fieldIndex,
			Object options, int i);

	/** Gets the title/label of option 'i'. */
	public abstract String formatOptionTitle(RecordFormatter rf, int fieldIndex,
			Object options, int i);

	/** Formats an option value, in the sequence of options. */
	public abstract String formatOptionValue(RecordFormatter rf, int fieldIndex,
	...
	public String format(RecordFormatter rf, int fieldIndex, Object o,
			Dictionary formatParams) {
		...
		if (!hidden) {
			HtmlChoiceWriter hcw = new HtmlChoiceWriter(getInputName(rf,
					fieldIndex, formatParams));
			...
			hcw
					.setLiteralHtml(getExtraFormatting(rf, fieldIndex,
							formatParams));
			...
			for (int i = 0; i < value.size(); i++) {
				valueFormattedList[i] = formatOptionValue(rf, fieldIndex, value
						.get(i));
			}
			...
			for (Enumeration f = value.elements(); f.hasMoreElements();) {
				...
				sb.append("<input type=\"hidden\" name=\"").append(
						getInputName(rf, fieldIndex, formatParams)).append(
	...
	/**
	 * Return value if not null, or finds the default option and returns it as a
	 * Vector.
	 */
