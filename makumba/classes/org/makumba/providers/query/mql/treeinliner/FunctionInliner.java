...
public class FunctionInliner {
    ...
    private AST inline(String query) {
        ...
        try {
            ...
            QueryAnalysisProvider.doThrow(query, t, ast);
        }
    ...
    private boolean inline(AST ast, boolean root) throws Throwable {
        ...
        // as we will need to treat the parameters again before normal analysis
        // we perform the analsysis on a duplicaed tree where we process the OQL parameters

        AST analysisAST = fact.dupTree(ast);
        ...
        InlinerMqlSqlWalker mqlAnalyzer = new InlinerMqlSqlWalker(query, null, true, true);
        ...
        if (mqlAnalyzer.getError() != null) {
            mqlAnalyzer.getError().printStackTrace();
            throw new Throwable(mqlAnalyzer.getError());
        }

        // now we traverse all the function calls
        if (mqlAnalyzer.getOrderedFunctionCalls().isEmpty()) {
            return false;
        } else {

            // re-order function calls
            mqlAnalyzer.setOrderedFunctionCalls(getOrderedFunctionCalls(mqlAnalyzer));
            if (debug) {
                System.out.println("Function calls:");
                for (FunctionCall f : mqlAnalyzer.getOrderedFunctionCalls().values()) {
                    System.out.println(f.getKey());
                }
            }

            int index = 0;
            for (FunctionCall c : mqlAnalyzer.getOrderedFunctionCalls().values()) {
                ...
                // if this is an actor function call (and not an inlining call), we need to inline it differently than
                // other functions
            ...
            while (originalRange != null) {
                ...
                boolean isTemporary = originalRange.getType() == HqlSqlTokenTypes.JOIN
                        && ASTUtil.getPath(originalRange.getFirstChild()).startsWith(TEMPORARY_LABEL)
    ...
    /**
     * The analyser inverts the function calls of functions that are arguments of another function, but as this doesn't
     * happen in the first-pass method-calls we re-order them here
     */
    private LinkedHashMap<String, FunctionCall> getOrderedFunctionCalls(InlinerMqlSqlWalker mqlAnalyzer) {
        ...
        for (String key : mqlAnalyzer.getOrderedFunctionCalls().keySet()) {
            FunctionCall c = mqlAnalyzer.getOrderedFunctionCalls().get(key);
            if (c.isFunctionArgument()) {
                f.add(c);
                mqlAnalyzer.getOrderedFunctionCalls().remove(c);
            } else {
    ...
    private void replaceArgsAndThis(AST tree, AST root, FunctionCall c, FunctionCall parentCall, AST parent,
            InlinerMqlSqlWalker mqlAnalyzer, boolean firstChild, String lastAlias, AST currentFrom) throws Throwable {
        ...
        switch (tree.getType()) {
            ...
            case HqlTokenTypes.IDENT:
                ...
                // avoid infinite recursion in case of already replaced arguments
                if (parent.getType() != HqlSqlTokenTypes.COLON) {
                ...
                if (tree.getText().equals("this")) {
                    ...
                    boolean isParameter = arg.getType() == HqlSqlTokenTypes.NAMED_PARAM
                            || arg.getType() == HqlSqlTokenTypes.PARAM || arg.getType() == HqlSqlTokenTypes.IDENT
     ...
     * 
     * @param tree
     *            the query fragment tree
     * @param c
     *            the {@link FunctionCall} we're inlining into
    ...
    private String createFunctionContext(AST tree, FunctionCall c) {
        ...
        if (a != null && a.getText().equals("makumbaGeneratedAlias")) {
            ...
            String parentType = isInSubfield ? c.getParentType().getParentField().getDataDefinition().getName()
                    : c.getParentType().getName();
    ...
    private void replaceFunctionArgument(AST root, FunctionCall c, AST parent, InlinerMqlSqlWalker mqlAnalyzer,
        ...
        FunctionCall cArg = mqlAnalyzer.getOrderedFunctionCalls().get(key);
        ...
        InlinerMqlSqlWalker analyzer = new InlinerMqlSqlWalker(query, null, true, true);
        ...
        MqlNode t = analyzer.getRootContext().getProjectionLabelSearch().get(projectionLabel);
    ...
    private void processActorFunction(FunctionCall c, AST queryFragmentTree, MethodCall methodCall) {
        ...
        if (methodCall.getParent().getType() == HqlTokenTypes.DOT && methodCall.isFirstChild()) {
            ...
            if (debug) {
                System.out.println("FunctionInliner.processActorFunction(): we have the following actor function, path is '"
                        + c.getPath() + "'");
     ...
     * 
     * @param temporary
     *            whether the FROM elements (RANGE, JOIN) of this label should be discarded on cleanup
     */
