...
public class MakumbaInfoTag extends TagSupport {
	public int doStartTag() throws JspException {
		try {
			pageContext.getOut().print("<style type=\"text/css\"> h1,h2,h3,h4 {color: blue; margin-bottom:0px}</style>");
...
//TODO detect initial memory and hotspot property 
			...
			try {
				...
				pageContext.getOut().print(
						" <p>could connect to /manager/list </p>");
			}

			pageContext.getOut().print("<h1>System Information</h1><table border=0 bgcolor=\"white\"><tr><td valign=top><font size=+1><b>Client: <td>Browser:");
			...
			pageContext.getOut().print("<table border=\"0\" cellspacing=0 cellpadding=0><tr><td>Initial heap (-Xms):&nbsp;<td align=right>");
			...
			//try{
			pageContext.getOut().print(ReadableFormatter.readableAge ((new Date()).getTime() - new Date(System.getProperty(startupProp)).getTime()));
		...
		//	}
			...
			pageContext.getOut().print("(");
			pageContext.getOut().print(ReadableFormatter.readableAge((new Date()).getTime()- MakumbaSystem.loadingTime.getTime()));
			...
			pageContext.getOut().print("<br></table><p><h2><a href=\"/makumba-docs/\"> Makumba</a>: </h2><table border=\"0\" cellspacing=0 cellpadding=0><tr bgcolor=\"#cccccc\"><Th>Property<th>Value");
			...
			pageContext.getOut().print(MakumbaSystem.getDatabaseProperty(dbname, "sql_engine.name")+ " "+ MakumbaSystem.getDatabaseProperty(dbname,"sql_engine.version"));
			pageContext.getOut().print("</code>");
			line++;
			pageContext.getOut().print("<tr bgcolor=\"#");
			if (line % 2 == 0) {
			...
			pageContext.getOut().print(MakumbaSystem.getDatabaseProperty(dbname,"jdbc_driver.name")+ " "+ MakumbaSystem.getDatabaseProperty(dbname,"jdbc_driver.version"));
			pageContext.getOut().print("</code></table><h3>Makumba caches: </h3><table border=\"0\" cellspacing=5 cellpadding=0><tr><th> Name <th>size<th>hits<th>misses</th>");
			line = 0;
			...
			pageContext.getOut().print(ReadableFormatter.readableAge((new Date()).getTime() - session.getCreationTime()));
			pageContext.getOut().print(" ago (");
			pageContext.getOut().print(session.getCreationTime());
			pageContext.getOut().print(")<br>Last Accessed:");
			pageContext.getOut().print(ReadableFormatter.readableAge((new Date()).getTime() - session.getLastAccessedTime()));
			pageContext.getOut().print(" ago (");
			pageContext.getOut().print(session.getLastAccessedTime());
			pageContext.getOut().print(")<br>Max inactive interval:");
			pageContext.getOut().print(ReadableFormatter.readableAge(session.getMaxInactiveInterval() * 1000));pageContext.getOut().print("(");
			...
			pageContext.getOut().print("<table border=\"0\" cellspacing=0 cellpadding=0><tr bgcolor=\"#cccccc\"><Th>Attribute<th>Value<th>Class");
			...
			pageContext.getOut().print("<table border=\"0\" cellspacing=0 cellpadding=0><tr bgcolor=\"#cccccc\"><Th>Property<th>Value");
			...
			while (enprop.hasMoreElements()) {
				...
				pageContext.getOut().print(":&nbsp;<td><pre>");
				pageContext.getOut().print(
						key.endsWith("path") ? sysprops.getProperty(key)
								.replace(
										sysprops.getProperty("path.separator")
												.charAt(0), '\n')
								: (((String) sysprops.getProperty(key))
										.startsWith("http://")) ? "<a href="
										+ sysprops.getProperty(key) + ">"
										+ sysprops.getProperty(key) + "</a>"
										: sysprops.getProperty(key));
				pageContext.getOut().print("</tr>");
			}
