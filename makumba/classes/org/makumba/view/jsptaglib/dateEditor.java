...
public class dateEditor extends FieldEditor
{
  ...
  String getNullName() { return getInputName()+"_null"; }
  String getComponentName(int i) { return getInputName()+"_"+i; }
  ...
  public String format(Object o, Dictionary formatParams)
  {
	...
	sb.append("<input type=hidden name=\"").append(getNullName()).append("\">");
      }
    int n=0;
    while(true)
      {
	...
	if(n==-1)
	  break;
	n=formatFrom(sb, d, format, n, hidden);
      }

    return sb.toString();
  }

  void formatComponent(StringBuffer sb, Date d, String fmt, int component, boolean hidden)
  {
  ...
  public Object readFrom(HttpParameters pr) 
  {
    Calendar c= new GregorianCalendar(org.makumba.MakumbaSystem.getTimeZone());
    c.clear();
    for(int i=0; i<components.length; i++)
      {
	String name=getComponentName(i);
    ...
    Date d=c.getTime();
    if(d.equals(getDefaultValue()) && pr.getParameter(getNullName())!=null)	
      return null;
    return d;
  }

  int formatFrom(StringBuffer sb, Date d, String format, int n, boolean hidden)
  {
    int m=n;
    char c= format.charAt(n);
    while(++n<format.length() && format.charAt(n)==c);
    formatComponent(sb, d, format.substring(m, n), recognized.indexOf(c), hidden);
    return n;
  }
