...
public class MDDPostProcessorWalker extends MDDPostProcessorBaseWalker {
    ...
    public MDDPostProcessorWalker(String typeName, MDDNode mdd, HashMap<String, FieldNode> typeShorthands,
            MDDFactory factory) {
    ...
    protected void processValidationDefinitions(ValidationRuleNode v, AST v_in) {
        if (v instanceof MultiUniquenessValidationRule) {
            ...
            if (!rhs_type.equals(rhs_type)) {
                factory.doThrow(typeName, "Invalid comparison expression: left-hand side type is "
                        + lhs_type.name().toLowerCase() + ", right-hand side type is " + rhs_type.name().toLowerCase(),
                    ce);
            }
            ...
            for (String arg : v.arguments) {
                if (!ce.getLhs().equals(arg) && !ce.getRhs().equals(arg)) {
                    factory.doThrow(typeName, "Argument '" + arg + "' is not being used in the comparison expression",
                        ce);
                }
            }
            ...
            if (ce.getLhs_type() == PATH && !v.arguments.contains(ce.getLhs())) {
                factory.doThrow(typeName, "Field '" + ce.getLhs() + "' not an argument of the comparison expression",
                    ce);
            }

            if (ce.getRhs_type() == PATH && !v.arguments.contains(ce.getRhs())) {
                factory.doThrow(typeName, "Field '" + ce.getRhs() + "' not an argument of the comparison expression",
                    ce);
            }
        }
