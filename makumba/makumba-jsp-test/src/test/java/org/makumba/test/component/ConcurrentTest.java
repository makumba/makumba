...
public class ConcurrentTest extends TestCase {
	...
	public static Test suite() {
		return setup = new MakumbaWebTestSetup(new TestSuite(
				ConcurrentTest.class), "oql");
	}
				...
				public void run() {
					try {
						...
						try {
							...
							GetMethod getTestPage = new GetMethod(
									System.getProperty("cactus.contextURL")
											+ CONCURRENT_TEST_URL);
							httpClient.executeMethod(getTestPage);
							String result = getTestPage
									.getResponseBodyAsString();
							// System.out.println(result);
							assertTrue(result
									.indexOf(EXPECTED_TEST_RESULT_FRAGMENT) > -1);
	...
	private void checkListPageAvailable() throws IOException, HttpException {
		HttpClient httpClient = new HttpClient();
		GetMethod getTestPage = new GetMethod(
				System.getProperty("cactus.contextURL") + CONCURRENT_TEST_URL);
		int status = -1;
		try {
		...
		assertEquals("Test page did not run correctly", HttpStatus.SC_OK,
				status);
	}
