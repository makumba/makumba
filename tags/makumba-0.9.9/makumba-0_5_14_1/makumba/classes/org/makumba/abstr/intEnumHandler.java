...
public class intEnumHandler extends intHandler implements intEnumerator
{
  ...
  public boolean isAssignableFrom(FieldDefinition fi){ return super.isAssignableFrom(fi) || fi.getType().equals("int") || fi.getType().equals("char"); }
  ...
  public String getNameFor(int n){ 
    ...
    throw new org.makumba.InvalidValueException(getFieldDefinition(), "Can't find a name for "+n+" in "+values+" with names "+names);
  }
  ...
  public Object checkValueImpl(Object value) 
  {
	...
	for(int i=0; i<values.size(); i++)
	  if(values.elementAt(i).equals(value))
	    return value;
	throw new org.makumba.InvalidValueException(getFieldDefinition(), "int value set to int enumerator ("+value+") is not a member of "+values);
      }
    if(!(value instanceof String))
      throw new org.makumba.InvalidValueException(getFieldDefinition(), "int enumerators only accept values of type Integer or String. Value supplied ("+value+") is of type "+value.getClass().getName());

    for(int i=0; i<names.size(); i++)
      if(names.elementAt(i).equals(value))
	return values.elementAt(i);
    
    throw new org.makumba.InvalidValueException(getFieldDefinition(), "string value set to int enumerator ("+value+") is neither a member of "+names+" nor amember of "+values);
  }
}
