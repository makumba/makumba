...
public class RecordParser {
	...
	void parse(RecordInfo dd) {
		...
		try {
			...
			dd.addStandardFields(dd.name
					.substring(dd.name.lastIndexOf('.') + 1));
			parse();
		} catch (RuntimeException e) {
			throw new MakumbaError(e, "Internal error in parser while parsing "
					+ dd.getName());
		}
	...
	RecordInfo parse(String txt) {
		...
		try {
			// make the default pointers resulted from the table name
			((RecordInfo) dd).addStandardFields(dd.getName().substring(
					dd.getName().lastIndexOf('.') + 1));
			parse();
		} catch (RuntimeException e) {
			throw new MakumbaError(e, "Internal error in parser while parsing "
					+ dd.getName());
	...
	static public java.net.URL findDataDefinitionOrDirectory(String s,
			String ext) {
		String s1 = null;
		java.net.URL u = null;
		if (s.startsWith("/"))
	...
	void solveIncludes() {
		...
		for (Enumeration e = text.keys(); e.hasMoreElements(); line++) {
			...
			if (st.startsWith("!include")) {
				...
				if (u == null) {
					mpe.add(fail("could not find include file " + s, ok + "="
							+ incl));
					return;
				}
				try {
					...
					mpe.add(fail(
							"could not find include file " + s + " " + ioe, ok
									+ "=" + incl));
					;
					return;
				}
				...
				for (Enumeration k = inclText.keys(); k.hasMoreElements();) {
					...
					if (val == null) // new field, not overriden in main mdd
						text.putAt(++line, key, inclText.getOriginal(key),
								inclText.getProperty(key));
					else
	...
	void treatMyFields() {
		...
		for (Enumeration e = fields.keys(); e.hasMoreElements(); line++) {
			...
			for (int i = 0; i < nm.length(); i++) {
				if (i == 0 && !Character.isJavaIdentifierStart(nm.charAt(i))
						|| i > 0
						&& !Character.isJavaIdentifierPart(nm.charAt(i)))
					mpe.add(fail("Invalid character \"" + nm.charAt(i)
							+ "\" in field name \"" + nm + "\"", nm));
			}
	...
	void configSubfields() {
		...
		for (Enumeration e = subfields.keys(); e.hasMoreElements();) {
			...
			p = nm.indexOf("->");
			String type = (String) getFieldInfo(nm.substring(0, p)).type;
			if (type == null) {
				mpe.add(fail("no such field in subfield definition", makeLine(
						subfields, nm)));
				continue;
			}

			String s;
			if ((s = addText(nm.substring(0, p), nm.substring(p + 2), subfields
					.getOriginal(nm), subfields.getProperty(nm))) != null)
	...
	DataDefinitionParseError fail(String why, String where) {
		return new DataDefinitionParseError(dd.getName(), why, where, where
				.length());
	}
	...
	void read(OrderedProperties op, java.net.URL u) throws IOException {
		read(op, new BufferedReader(new InputStreamReader((InputStream) u
				.getContent())));
	}
	...
	public void parse_ptrOne_Subfields(String fieldName) {
		...
		getFieldInfo(fieldName).extra2 = ((RecordParser) ptrOne_RecordParsers
				.get(fieldName)).dd.getTitleFieldName();
	}

	//moved from setParser
	public void parse_set_Subfields(String fieldName) {
		if (getFieldInfo(fieldName).extra2 == null) {
			getFieldInfo(fieldName).extra2 = ((RecordInfo) subtableParser_subtables
					.get(fieldName)).title = ((DataDefinition) setParser_settbls
					.get(fieldName)).getTitleFieldName();
		}
	}

	//	moved from FieldParser
	String acceptTitle(String fieldName, String nm, String origNm, String val,
			Object o) {
		val = val.trim();
		if (nm.equals("!title")) {
	...
	String add_ptr_Text(String fieldName, String nm, String origNm, String val) {
		return acceptTitle(fieldName, nm, origNm, val,
				getFieldInfo(fieldName).extra1);
	}

	//moved from ptrOneParser
	String add_ptrOne_Text(String fieldName, String nm, String origNm,
			String val) {
	...
	String add_set_Text(String fieldName, String nm, String origNm, String val) {
		String s = acceptTitle(fieldName, nm, origNm, val,
				(DataDefinition) setParser_settbls.get(fieldName));
		if (s == null)
			((RecordInfo) subtableParser_subtables.get(fieldName)).title = val
					.trim();
		return s;
	}

	//	moved from FieldParser
	void parse(String fieldName, FieldCursor fc)
	...
	//	moved from FieldParser
	String setType(String fieldName, String type, FieldCursor fc)
			throws org.makumba.DataDefinitionParseError {
	...
	/**
	 * switch for the existing parse methods. set the field type to another
	...
	public void char_parse1(String fieldName, FieldCursor fc) {
		if (!fc.lookup("{")) {
			...
			if (size.intValue() > 255 || size.intValue() < 0)
				throw fc.fail("char size must be between 0 and 255, not "
						+ size.toString());
	...
	public void text_parse1(String fieldName, FieldCursor fc)
			throws org.makumba.DataDefinitionParseError {
	...
	public void ptrOne_parse1(String fieldName, FieldCursor fc) {
		...
		ptrOne_RecordParsers.put(fieldName, new RecordParser(
				((RecordInfo) subtableParser_subtables.get(fieldName)), this));
		return;
	}

	//moved from setEnumParser, setChatEnumParser
	public void setCharEnum_parse1(String fieldName, FieldCursor fc) {
		FieldInfo _enum = new FieldInfo(((RecordInfo) subtableParser_subtables
				.get(fieldName)), "enum");
		makeSubtable(fieldName, fc);
		((RecordInfo) subtableParser_subtables.get(fieldName)).mainPtr = addPtrHere(fieldName);
	...
	public void set_parse1(String fieldName, FieldCursor fc) {
		...
		if (ori == null) {
			...
			if (word == null) {
				try {
					getFieldInfo(fieldName).description = fc
							.lookupDescription();
				} catch (org.makumba.DataDefinitionParseError pe) {
					throw fc
							.fail("table specifier, enumeration type, or nothing expected");
				}
			...
			if (s == null)
				throw fc
						.fail("table, char{}, int{} or macro type expected after set");
		...
		((RecordInfo) subtableParser_subtables.get(fieldName)).setField = addPtr(
				fieldName, ((RecordInfo) setParser_settbls.get(fieldName))
						.getBaseName(), ori);
		return;
	}
	...
	void makeSubtable(String fieldName, FieldCursor fc) {
		...
		((RecordInfo) subtableParser_subtables.get(fieldName))
				.addStandardFields(((RecordInfo) subtableParser_subtables
						.get(fieldName)).subfield);
		getFieldInfo(fieldName).extra1 = ((RecordInfo) subtableParser_subtables
				.get(fieldName));
	}
	...
	String addPtr(String fieldName, String name, DataDefinition o) {
		...
		while (((RecordInfo) subtableParser_subtables.get(fieldName)).fields
				.get(name) != null)
			name = name + "_";

		FieldInfo ptr = new FieldInfo(((RecordInfo) subtableParser_subtables
				.get(fieldName)), name);
	...
	String addPtrHere(String fieldName) {
		...
		if (((RecordInfo) subtableParser_here.get(fieldName)).getParentField() != null)
			return addPtr(fieldName, ((RecordInfo) subtableParser_here
					.get(fieldName)).subfield,
					((RecordInfo) subtableParser_here.get(fieldName)));
		else
			return addPtr(fieldName, ((RecordInfo) subtableParser_here
					.get(fieldName)).name, ((RecordInfo) subtableParser_here
					.get(fieldName)));
	}
}
