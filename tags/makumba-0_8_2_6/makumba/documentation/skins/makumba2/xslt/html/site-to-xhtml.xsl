...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
  ...
  <xsl:include href="../../../common/xslt/html/tabutils.xsl"/>
<!--+
...
<!--+
  ...
  +--><link rel="stylesheet" href="{$root}skin/basic.css" type="text/css" />
      ...
      <link rel="stylesheet" href="{$root}skin/makumba.css" type="text/css" media="screen" />
<!--+
  |Javascripts
  +-->
...
<script type="text/javascript" language="javascript" 
				src="{$root}skin/myMenu.js"></script>
<script type="text/javascript" language="javascript" 
...
<!--breadcrumbs org location-->
    ...
    +</xsl:comment>
          <a href="/index.html">
            <div id="homeLink"><xsl:text> </xsl:text></div>
          </a>
          <div class="header">
            <xsl:if 
...
<!-- Lucene search -->
                    <form method="get" action="{$root}{$lucene-search}">
                      ...
                      <xsl:comment>+
      |Jasper: This button is obsolete if you have a graphical browser with a enter button
							      +</xsl:comment>                      
									  <noscript><input type="submit" value="Search" name="Search" i18n:attr="value"/></noscript>
                    </form>
                  </xsl:when>
                  <xsl:otherwise>
                    <form method="get" action="http://www.google.com/search">
                      <input type="hidden" name="sitesearch" value="{$config/search/@domain}"/>
                      <input type="text" id="query" name="q" onFocus="getBlank (this, '{$search-prompt}');">
                        <xsl:attribute name="value">
                          <xsl:value-of select="$search-prompt"/>
                        </xsl:attribute>
                      </input>&#160; 
...
<!--+
  ...
  +-->
        ...
        <!-- Don't need this...  -->
    <xsl:comment>+
    Jasper: Don't need this... </xsl:comment>
    <!-- 
...
<!--credits in alternative location-->
  ...
  <xsl:template name="innermenuli">
    ...
    <xsl:comment>+
    |Jasper: Some black magic to prevent the toplevel item to be expandable
    +</xsl:comment>
    <div class="menutitle" id="{$tagid}Title">
      <xsl:if test="$toplevel!='yes'">
	      <xsl:attribute name="onclick">
	       <xsl:text>SwitchMenu('</xsl:text>
    ...
    <div class="{$whichGroup}" id="{$tagid}">
      <xsl:attribute name="class">
        <xsl:value-of select="$whichGroup"/>
        <xsl:choose>
    ...
    <xsl:if test="$config/toc">
      <xsl:if test="contains($minitoc-location,'page')">
        <xsl:if test="(count(//tocitems/tocitem) >= $config/toc/@min-sections) or (//tocitems/@force = 'true')">
          <div id="minitoc-area">
            <h3>
             Contents [<a id="togglelink" href="javascript:toggleToc()">Hide</a>]</h3>
