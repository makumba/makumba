...
/**
 * MQL implementation of the {@link ParameterTransformer}, which generates SQL
 * based on a {@link QueryAnalysis}
 * 
 * @author Manuel Bernhardt <manuel@makumba.org>
 * @version $Id: MqlSQLQueryGenerator.java,v 1.1 Mar 30, 2010 4:17:00 PM manu
 *          Exp $
 */
public class MqlParameterTransformer implements ParameterTransformer {
    ...
    public int getParameterCount() {
        if (expandedParamInfo == null) {
            throw new MakumbaError(
                    "Can't call this method without having set the arguments with setArguments!");
        } else {
    ...
    /**
     * Expands multiple parameters, i.e. parameters that are vectors or lists.
     * This is necessary for execution of the SQL query. This method expands the
     * analyser tree and multiplies the parameters according the size of the
     * multiple parameters, and sets the expandedParamInfo so that clients of
     * the {@link ParameterTransformer} can use it to do type checking on the
     * SQL query parameters.
     */
    private void expandMultipleParameters(Map<String, Object> arguments)
            throws ProgrammerError {

        expandedParamInfo = DataDefinitionProvider
                .getInstance()
                .getVirtualDataDefinition("SQL parameters for " + qA.getQuery());

        ArrayList<AST> queryParams = findQueryParameters(analyserTreeSQL,
                new ArrayList<AST>());

        // expand multiple params (vectors, lists) into multiple parameter
        // entries
        for (int i = 0; i < qA.getParameterOrder().size(); i++) {
            Object val = getArgumentValue(qA.getParameterOrder().get(i),
                    arguments);
            ...
            if (val instanceof List<?>) {
                ...
                for (int j = 0; j < v.size() - 1; j++) {
                    ...
                    // expand tree
                    qp.setNextSibling(ASTUtil.create(qA.getAnalyserFactory(),
                            HqlSqlTokenTypes.NAMED_PARAM, "?"));
                ...
                for (int k = 0; k < v.size(); k++) {
                    expandedParamInfo.addField(DataDefinitionProvider
                            .getInstance().makeFieldWithName(
                                    fd.getName() + "_" + k, fd));
                }
            } else {
                expandedParamInfo.addField(DataDefinitionProvider.getInstance()
                        .makeFieldWithName(fd.getName(), fd));
            }
        }
    }

    /**
     * Gets the value of a given argument, applies name transformation if
     * necessary, and checks if the value is not null
     * 
     * @param argumentName
     ...
     */
    private Object getArgumentValue(String argumentName,
            Map<String, Object> arguments) throws ProgrammerError {
        ...
        if (val == null) {
            throw new ProgrammerError("The parameter '" + argumentName
                    + "' should not be null");
        }
        return val;
    }
    ...
    static String getActualArgumentName(String argumentName) {
        if (argumentName.startsWith(MqlQueryAnalysis.MAKUMBA_PARAM)) {
            argumentName = argumentName
                    .substring(MqlQueryAnalysis.MAKUMBA_PARAM.length());
            int n = Integer.parseInt(argumentName);
            argumentName = "" + (n + 1);
        }
        if (argumentName.indexOf("###") > 0) {
            argumentName = argumentName.substring(0,
                    argumentName.indexOf("###"));
        }
        return argumentName;
    }

    /**
     * Find all the named parameters in the analysis tree and puts them in a
     * list
     */
    ...
    /**
     * Rudi: I did not code this method, and don't yet fully understand it's
     * purpose. Also, the name seems wrong, as there is no positional
     * information at all.<br/>
     * This method does a checking whether the given value is acceptable for the
     * type of the given {@link FieldDefinition}. This method seems very similar
     * to what {@link FieldDefinition#checkValue(Object)} does, but is less
     * strict on pointers, it doesn't check whether the pointer belongs to the
     * correct MDD.<br/>
     * A bit strangely, the method returns false *only* for
     * {@link FieldDefinition} that are a "multiTypeParam".
     */
    public static boolean isValueInvalidForPosition(FieldDefinition fd,
            Object value) {
        ...
        boolean isDifferentPointer = fd.isPointer()
                && value instanceof Pointer
                && !fd.getPointedType().getName()
                        .equals(((Pointer) value).getType());
        boolean isNumber = (fd.isIntegerType() || fd.isRealType())
                && !(value instanceof Number);
        boolean giveUp = isMultiTypeParam
                && (isChar || isPointer || isNumber || isDifferentPointer);
        return giveUp;
    }

    private static int generators = NamedResources.makeStaticCache(
            "SQL Query Generators", new NamedResourceFactory() {
                ...
                @Override
                protected void configureResource(Object name, Object hashName,
                        Object resource) throws Throwable {
                    Object[] multi = (Object[]) name;
                    @SuppressWarnings("unchecked")
                    Map<String, Object> args = (Map<String, Object>) multi[1];
    ...
    public static MqlParameterTransformer getSQLQueryGenerator(
            MqlQueryAnalysis qA, Map<String, Object> args) {
        ...
        return (MqlParameterTransformer) NamedResources.getStaticCache(
                generators).getResource(new Object[] { qA, args });
    ...
    public static void main(String[] args) {
        ...
        for (String n : qG.getTransformedParameterTypes().getFieldNames()) {
            System.out.println(qG.getTransformedParameterTypes()
                    .getFieldDefinition(n));
        }
    }
