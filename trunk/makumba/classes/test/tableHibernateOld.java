...
public class tableHibernateOld extends TestCase {
    ...
    static String[] PersonOldFields = { "TS_modify", "TS_create", "extraData", "birthdate" };
    ...
    public void testDelete() {
        ...
        assertEquals(0, db.executeQuery("SELECT l.id FROM  test.PersonOld p JOIN p.speaks l WHERE p.id=?", ptr).size());
        assertEquals(0,
            db.executeQuery("SELECT l.enum_ FROM  test.PersonOld p JOIN p.intSet l WHERE p.id=?", ptr).size());
        assertEquals(0,
            db.executeQuery("SELECT l.enum_ FROM  test.PersonOld p JOIN p.charSet l WHERE p.id=?", ptr).size());
