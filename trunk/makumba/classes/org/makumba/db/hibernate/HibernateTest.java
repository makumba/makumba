...
public class HibernateTest {
    ...
    public static void main(String[] args) {
        ...
        // SessionFactory sf = HibernateSFManager.getSF(dds, "dataDefinitions",
        ...
        // Query q =
        // session.createQuery("SELECT i.surname, p.weight FROM test.Person p, IN(p.indiv) i WHERE i.name = 'Bart'");
        ...
        // Query q =
        // session.createQuery("SELECT a.streetno FROM test.Person p JOIN p.address a WHERE p.indiv.name = 'Bart'");

        // a manual pointer join
        // Query q =
        // session.createQuery("SELECT i.name, p.weight FROM test.Person p, test.Individual i WHERE p.indiv = i");
