...
public class MultipartHttpParameters extends HttpParameters {
    ...
    public MultipartHttpParameters(HttpServletRequest req) {
        ...
        try {
            ...
            int var2 = pis.read();
            if (var1 != 13 || var2 != 10)
                MakumbaSystem.getMakumbaLogger("fileUpload").severe(
                        "Mul: ERROR reading CRLF after the 1st boundary [" + var1 + "][" + var2 + "]");

            // go through all the request
            endOfRequest: while (true) {
                ...
                while (true) {
                    ...
                    if (NrBytesRead > 2) {
                        ...
                        if (byteRead == 13 && (headers.charAt(NrBytesRead - 3)) == 13) {
                            x = pis.read(); // if != 10
                            // some browsers send CR LF CR CR so no -> throw new org.makumba.MakumbaError("Multipart Not
                            // recognized");
                            MakumbaSystem.getMakumbaLogger("fileUpload").fine(
                                    "MUL-endOfHeaders: [" + (int) headers.charAt(NrBytesRead - 3) + "|"
                                            + (int) headers.charAt(NrBytesRead - 2) + "] [" + byteRead + "|" + x + "]");
                ...
                if (index_h != -1) {
                    ...
                    MakumbaSystem.getMakumbaLogger("fileUpload").warning(
                            "DEBUG-mul: ------ no \"name\" in the headers was found \n\n");
                ...
                if (index_h != -1) {// fileName
                    ...
                    if (fileName.length() > 0) {
                        ...
                        if (index_h != -1) {
                            ...
                            if (type.indexOf("application/x-macbinary") != -1) {
                                MakumbaSystem.getMakumbaLogger("fileUpload")
                                        .fine("Mac upload: application/x-macbinary");
                            ...
                            MakumbaSystem.getMakumbaLogger("fileUpload").warning(
                                    "no Content-Type found but there might be some content");
                        }
                        ...
                        if (checkContent.length() > 0)
                            MakumbaSystem.getMakumbaLogger("fileUpload").warning(
                                    "no filename in the headers but there was some content (size="
                                            + checkContent.length() + ", that was not put in the parameters");
                    ...
                    // no filename so it is a string parameter. We store it as String
