...
public class ValueTag extends GenericListTag {
    ...
    public int doAnalyzedStartTag(PageCache pageCache) throws JspException, org.makumba.LogicException {
        ...
        if (StringUtils.equals(editable, "true")) {
            ...
            // we first check if we have a editable type (this will need to be changed, later we will also support dates and stuff)
            ...
            if(fd.getIntegerType() == FieldDefinition._char || fd.getIntegerType() == FieldDefinition._int || fd.getIntegerType() == FieldDefinition._text) {
                ...
                if (parentExpr != null) {
                    ...
                    parentObjectKey.remove(tagKey.size() - 1);
                    parentObjectKey.add(parentExpr);
                    ptr = (Pointer) ((ValueComputer) pageCache.retrieve(MakumbaJspAnalyzer.VALUE_COMPUTERS, parentObjectKey)).getValue(pageContext);
                ...
                sb.append("       <form method='post' class='mak_edit' action='" + request.getContextPath()
                        + "/valueEditor?queryLanguage=" + MakumbaJspAnalyzer.getQueryLanguage(pageCache) + "&logicClass=" + logicClass + "&editPage=" + editPage + "&table=" + ptr.getType() + "&field=" + fieldName + "&pointer=" + externalPointer
                        + "'");
                ...
                switch (fd.getIntegerType()) {
                    ...
                    case FieldDefinition._int:
                        sb.append("               <input type='hidden' id='mak_edittype_" + counter + "' value='int' />\n");
                        sb.append("               <input type='text' name='value' id='mak_onedit_" + counter
                                + "_text' value='' />\n");
                        sb.append("               <input type='submit' value='X' />\n");
                        sb.append("       </form>\n");
