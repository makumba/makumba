...
public class MDDParser extends MDDBaseParser {
    ...
    protected void postProcess() {
        ...
        for (String key : fieldsToDisable.keySet()) {
            factory.doThrow(this.typeName, "Fields cannot have an empty body unless they override an included field",
                fieldsToDisable.get(key));
        }
    }
    ...
    protected AST parseFunctionBody(AST expression) {
        ...
        String query = "SELECT " + (subquery ? "(" : "") + expression.getText() + (subquery ? ")" : "") + " FROM "
                + typeName + " makumbaGeneratedAlias";
    ...
    protected void checkFieldName(AST fieldName) {
        ...
        for (int i = 0; i < nm.length(); i++) {
            if (i == 0 && !Character.isJavaIdentifierStart(nm.charAt(i)) || i > 0
                ...
                factory.doThrow(this.typeName, "Invalid character \"" + nm.charAt(i) + "\" in field name \"" + nm,
                    fieldName);
            }
        }
