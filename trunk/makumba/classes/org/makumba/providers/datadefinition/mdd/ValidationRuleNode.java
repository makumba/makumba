...
public class ValidationRuleNode extends MDDAST implements ValidationRule, Comparable<ValidationRule> {
    ...
    /**
     * field node the rule applies to. this is only used for single-field validation rules, as convenience mechanism to
     * check the rule applicability. TODO maybe drop this in favor of using the arguments
     **/
    ...
    public void checkApplicability() {
        if (!type.checkApplicability(field.makumbaType)) {
            throw new RuntimeException("A " + getValidationType().getDescription()
                    + " can only be assigned to fields of type " + getValidationType().getApplicableTypes()
                    + " whereas the type of field " + field.name + " is " + field.makumbaType.getTypeName());
        }
    }
